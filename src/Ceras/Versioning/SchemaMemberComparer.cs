namespace Ceras
{
	 ...
	 * - Must have stable order (which .NET/C# do *not* ensure on their own)
	...
	class SchemaMemberComparer : IComparer<SchemaMember>
	{
		...
		static string GetComparisonName(SchemaMember m)
		{
			...
			return
				...
				// todo: not yet enabled as we'd need to invest more time in dynamicFormatter; also will probably break the current protocol
				(IsFixedSize(m.MemberType) ? "" : "") + 
